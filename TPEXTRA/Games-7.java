...
class Games {
    ...
    static class NoAN {
        ...
        public NoAN(Games jogo) {
            this.jogo = jogo;
            this.cor = true; // novo nó nasce vermelho
            this.esq = null;
            this.dir = null;
        }
    }
    ...
    static class ArvoreAlvinegra {
        ...
        public void inserir(Games g) {
            raiz = inserir(g, raiz);
            raiz.cor = PRETO; // raiz deve ser preta
        }
    
        private NoAN inserir(Games g, NoAN h) {
            if (h == null) return new NoAN(g);
    
            int cmp = g.getNome().compareToIgnoreCase(h.jogo.getNome());
            ...
            // --------- balanceamentos ----------
            if (isVermelho(h.dir) && !isVermelho(h.esq)) 
                h = rotacaoEsq(h);
    
            if (isVermelho(h.esq) && isVermelho(h.esq.esq))
                h = rotacaoDir(h);
    
            if (isVermelho(h.esq) && isVermelho(h.dir))
                trocarCores(h);
    
            return h;
        }
    
        private boolean isVermelho(NoAN x) {
        ...
        private void trocarCores(NoAN h) {
            h.cor = VERMELHO;
            if (h.esq != null) h.esq.cor = PRETO;
            if (h.dir != null) h.dir.cor = PRETO;
        }
        ...
        private boolean pesquisar(String nome, NoAN i) {
            ...
            int cmp = nome.compareToIgnoreCase(i.jogo.getNome());
