...
public class FilaFlexivel {
    ...
    public void mostrar() {
        int i = 0;
        for (Celula c = primeiro.prox; c != null; c = c.prox, i++) {
            System.out.print("[" + i + "] ");
            Games.Printar(c.elemento);
        }
    }

    // ------------------- MAIN -------------------
    public static void main(String[] args) {
        File arq = new File(Games.FILE_PATH);
        Games[] catalogo = Games.SetCatalogo(Games.LeituraCsv(arq));
        FilaFlexivel fila = new FilaFlexivel();
        Scanner sc = new Scanner(System.in);

        // IDs iniciais até FIM
        String entrada;
        ...
        for (int i = 0; i < n; i++) {
            ...
            switch (partes[0]) {
                case "IF":
                    fila.inserir(Games.PesquisaSeqMod(catalogo, Integer.parseInt(partes[1])));
                    break;
                case "R": // <- corrigido: comando certo para remover
                    Games removido = fila.remover();
                    if (removido != null)
