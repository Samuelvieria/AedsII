...
public class Games {
    ...
    private static int compare(Games g1, Games g2) {
        comparacoes++;
        if (g1.getPreco() != g2.getPreco()) {
            return Float.compare(g1.getPreco(), g2.getPreco());
        } else {
            return Integer.compare(g1.getId(), g2.getId());
        }
    }

    public static void mergesort(Games[] catalogo) {
        Games[] merge = ArrayTemp(catalogo);
        int n = merge.length;
        ...
        for (int k = merge.length - 1; k >= merge.length - 5; k--) {
            Printar(merge[k]);
        }
        ...
        // Gerar arquivo de log
        String matricula = "800772";
        try (PrintWriter writer = new PrintWriter(matricula + "_mergesort.txt")) {
            writer.printf("%s\t%d\t%d\t%.2f\n", matricula, comparacoes, movimentacoes, executionTimeMs);
        } catch (Exception e) {
            System.out.println("Erro ao criar arquivo de log: " + e.getMessage());
        }
    ...
    private static void mergesort(int esq, int dir, Games[] merge) {
        if (esq < dir) {
            int meio = (esq + dir) / 2;
    ...
    private static void intercalar(int esq, int meio, int dir, Games[] merge) {
        int n1 = meio - esq + 1;
        int n2 = dir - meio;

        Games[] a1 = new Games[n1];
        Games[] a2 = new Games[n2];
