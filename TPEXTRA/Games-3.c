...
// Função para comparar duas datas (Release_date) com desempate por AppID
static int comparar_games(const Game *a, const Game *b) {
    comparacoes++;
    if (a->date.ano != b->date.ano) return a->date.ano - b->date.ano;
    if (a->date.mes != b->date.mes) return a->date.mes - b->date.mes;
    if (a->date.dia != b->date.dia) return a->date.dia - b->date.dia;
    return a->id - b->id;
}
...
static void quicksortRec(Game *array, int esq, int dir) {
    ...
    while (i <= j) {
        while (comparar_games(&array[i], &pivo) < 0) i++;
        while (comparar_games(&array[j], &pivo) > 0) j--;
        if (i <= j) {
...
// Função para criar um array temporário com jogos selecionados por ID (faz deep copy agora)
static Game *array_temp(Game *catalog, int catalog_count, int *out_count) {
    char entrada[256];
    Game *temp = malloc(sizeof(Game) * 200); // Array temporário com tamanho máximo inicial
    if (!temp) {
        perror("malloc temp");
        *out_count = 0;
        return NULL;
    ...
    while (fgets(entrada, sizeof(entrada), stdin)) {
        ...
        if (found && i < 200) {
            // faz deep copy do jogo encontrado
    ...
    if (!jogos_tmp) {
        ...
        // liberar o que foi alocado em temp
...
int main(int argc, char **argv) {
    ...
    FILE *log_file = fopen("800772_quicksort.txt", "w");
    if (log_file) {
    ...
    // Liberar memória do array temporário (agora safe, porque são deep copies)
    for (int i = 0; i < temp_count; i++) {
    ...
    // Liberar memória do catálogo original
    for (int i = 0; i < catalog_count; i++) {
        free_game(&catalog[i]);
    }
    free(catalog);
