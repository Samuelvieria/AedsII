...
int compareShowsByTypeThenTitle(const void* a, const void* b) {
    const Show* showA = *(const Show**)a;
    const Show* showB = *(const Show**)b;
    
    // Primeiro compara por type
    int cmpType = strcmp(showA->type, showB->type);
    if (cmpType != 0) {
        return cmpType;
    }
...
int main() {
    ...
    // Ordenar shows por type e depois por title
    qsort(showsSelecionados, numSelecionados, sizeof(Show*), compareShowsByTypeThenTitle);
