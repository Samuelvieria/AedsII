...
void ler(Show* show, const char* linha) {
    ...
    parseDate(getOrNaN(campos, 6), &show->dataAdded);
...
void formatar(const Show* show, char* output) {
    ...
    // Formatar data
    char dataStr[100];
    if (show->dataAdded.tm_year == 0 && show->dataAdded.tm_mon == 0 && show->dataAdded.tm_mday == 0) {
        strcpy(dataStr, "NaN");
    } else {
        const char* months[] = {"January", "February", "March", "April", "May", "June", 
                               "July", "August", "September", "October", "November", "December"};
        sprintf(dataStr, "%s %d, %d", months[show->dataAdded.tm_mon], 
                show->dataAdded.tm_mday, show->dataAdded.tm_year + 1900);
    }
...
int compareShowsByDateThenTitle(const void* a, const void* b) {
    const Show* showA = *(const Show**)a;
    const Show* showB = *(const Show**)b;
    
    // Primeiro compara por data
    int cmpDate = compareDates(&showA->dataAdded, &showB->dataAdded);
    if (cmpDate != 0) {
        return cmpDate;
    }
    
    // Se datas forem iguais, compara por título
...
int main() {
    ...
    // Ordenar shows por data de adição e depois por título
    qsort(showsSelecionados, numSelecionados, sizeof(Show*), compareShowsByDateThenTitle);
