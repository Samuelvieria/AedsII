...
// --- Linked List Head (ListaShow) ---
typedef struct {
    Node* head;
    int n; // Current number of elements in the list
} ListaShow;
...
// Function to initialize ListaShow (linked list)
void initListaShow(ListaShow* lista) {
    lista->head = NULL;
    lista->n = 0;
}

// Function to free memory used by ListaShow (linked list)
void freeListaShow(ListaShow* lista) {
    Node* current = lista->head;
    Node* next;
    while (current != NULL) {
        next = current->next;
        freeShow(&current->data); // Free dynamic members of the Show data
...
void inserirInicio(ListaShow* lista, Show show) {
    ...
    newNode->data = copyShow(&show); // Deep copy the incoming show data
...
void inserir(ListaShow* lista, Show show, int pos) {
    if (pos < 0 || pos > lista->n) {
        fprintf(stderr, "Error: Posicao invalida (%d) - cannot insert.\n", pos);
        return;
    }
...
Show removerInicio(ListaShow* lista) {
    ...
    Show removedData = temp->data; // The data is already a deep copy inside the node
    lista->head = lista->head->next;
    free(temp); // Free the node itself
    lista->n--;
    return removedData; // Caller is responsible for freeing removedData's dynamic members
}
...
Show removerFim(ListaShow* lista) {
    ...
    if (lista->head->next == NULL) { // Only one node
    ...
    Show removedData = current->data; // The data is already a deep copy inside the node
    prev->next = NULL;
    free(current); // Free the node itself
    lista->n--;
    return removedData; // Caller is responsible for freeing removedData's dynamic members
...
Show remover(ListaShow* lista, int pos) {
    if (lista->head == NULL || pos < 0 || pos >= lista->n) {
        fprintf(stderr, "Error: Posicao invalida (%d) ou lista vazia (list empty) - cannot remove.\n", pos);
        Show emptyShow;
        initShow(&emptyShow);
        return emptyShow;
    }
...
int main() {
    ...
    ListaShow lista;
    initListaShow(&lista); // Initialize linked list
    ...
    while (scanf("%19s", id) == 1 && strcmp(id, "FIM") != 0) {
        ...
        if (s != NULL) {
            inserirFim(&lista, *s); // Insert into the linked list
        }
    }
    ...
    for (int i = 0; i < n_commands; i++) {
        ...
        if (strcmp(cmd, "II") == 0) { // Insert at beginning
            ...
            if (strcmp(removed.showId, "NaN") != 0) { // Check if a valid show was removed
                printf("(R) %s\n", removed.title);
    ...
    freeListaShow(&lista); // Frees elements in the list and the list nodes themselves
